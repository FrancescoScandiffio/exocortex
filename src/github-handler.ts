...
app.get("/authorize", async (c) => {
    ...
    return renderApprovalDialog(c.req.raw, {
        ...
        csrfToken,
        server: {
            description: "Exocortex MCP Server — persistent context for AI via GitHub.",
            name: "Exocortex",
        },
        setCookie,
        state: {oauthReqInfo},
...
app.get("/callback", async (c) => {
    ...
    // Reject unauthorized users before issuing any token
    const allowedUsers = new Set(c.env.ALLOWED_USERNAMES.split(",").map((s: string) => s.trim()));
    ...
    const {redirectTo} = await c.env.OAUTH_PROVIDER.completeAuthorization({
        ...
        // This will be available on this.props inside ExocortexMCP
        props: {
            accessToken,
            email,
            login,
